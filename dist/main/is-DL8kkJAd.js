...
var require_semver = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function SemVer(version, options) {
		...
		if (version.length > 256) throw new TypeError("version is longer than " + MAX_LENGTH + " characters");
	...
	SemVer.prototype.inc = function(release$1, identifier) {
		switch (release$1) {
			...
			case "pre":
				...
				break;
			default: throw new Error("invalid increment argument: " + release$1);
		}
		this.format();
	...
	exports.gt = gt$1;
	function gt$1(a, b, loose) {
		return compare(a, b, loose) > 0;
	}
	exports.lt = lt$1;
	function lt$1(a, b, loose) {
		return compare(a, b, loose) < 0;
	}
	exports.eq = eq;
	...
	function cmp(a, op, b, loose) {
		switch (op) {
			...
			case ">": return gt$1(a, b, loose);
			case ">=": return gte(a, b, loose);
			case "<": return lt$1(a, b, loose);
	...
	function minVersion(range, loose) {
		...
		for (var i$1 = 0; i$1 < range.set.length; ++i$1) range.set[i$1].forEach(function(comparator) {
			...
			switch (comparator.operator) {
				...
				case "":
				case ">=":
					if (!minver || gt$1(minver, compver)) minver = compver;
					break;
				case "<":
				case "<=": break;
...
var require_electron_is_dev = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var getFromEnv = parseInt(process.env.ELECTRON_IS_DEV, 10) === 1;
	module.exports = "ELECTRON_IS_DEV" in process.env ? getFromEnv : process.defaultApp || /node_modules[\\/]electron[\\/]/.test(process.execPath);
}));
var require_is = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var semver = require_semver();
	var gt = semver.gt;
	var lt = semver.lt;
	var release = __require("os").release;
	var isDev = require_electron_is_dev();
	module.exports = {
		renderer: function() {
		...
		all: function() {
			...
			for (var i$1 = 0; i$1 < isFunctions.length; i$1++) isFunctions[i$1] = arguments[i$1];
			if (!isFunctions.length) return;
			for (i$1 = 0; i$1 < isFunctions.length; i$1++) if (!isFunctions[i$1]()) return false;
			return true;
		},
		none: function() {
			const isFunctions = new Array(arguments.length);
		...
		one: function() {
			...
			for (var i$1 = 0; i$1 < isFunctions.length; i$1++) isFunctions[i$1] = arguments[i$1];
			if (!isFunctions.length) return;
			for (i$1 = 0; i$1 < isFunctions.length; i$1++) if (isFunctions[i$1]()) return true;
			return false;
		},
		release: function(requested) {
			if (this.osx()) return requested === osxRelease();
			else if (this.windows()) {
				requested = requested.split(".");
				const actual = release().split(".");
				if (requested.length === 2) return `${actual[0]}.${actual[1]}` === `${requested[0]}.${requested[1]}`;
				return `${actual[0]}.${actual[1]}.${actual[2]}` === `${requested[0]}.${requested[1]}.${requested[2]}`;
			} else return null;
		},
		gtRelease: function(requested) {
			if (this.osx()) return gt(requested, osxRelease());
			else if (this.windows()) {
				requested = requested.split(".");
				const actual = release().split(".");
				if (requested.length === 2) return gt(`${requested[0]}.${requested[1]}.0`, `${actual[0]}.${actual[1]}.0`);
				return gt(`${requested[0]}.${requested[1]}.${requested[2]}`, `${actual[0]}.${actual[1]}.${actual[2]}`);
			} else return null;
		},
		ltRelease: function(requested) {
			if (this.osx()) return lt(requested, osxRelease());
			else if (this.windows()) {
				requested = requested.split(".");
				const actual = release().split(".");
				if (requested.length === 2) return lt(`${requested[0]}.${requested[1]}.0`, `${actual[0]}.${actual[1]}.0`);
