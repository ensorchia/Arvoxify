...
const main = {
	"console": {
		"did-finish-load": { "dev-tools": "הטעינה הסתיימה. הכלים לפמתחים נפתחו" },
		"i18n": { "loaded": "i18n נטען" },
		"second-instance": { "receive-command": "התקבלה פקודה מעבר פרוטוקל: {{command}}" },
		"theme": { "css-file-not-found": "קובץ ה-CSS \"{{cssFile}}\" לא קיים. מדלג" },
		"unresponsive": { "details": "שגיאה ללא תגובה\n{{error}}" },
		"when-ready": { "clearing-cache-after-20s": "מוחק קבצי מתמון" },
		"window": { "tried-to-render-offscreen": "ווינדוס ניסה להציג תוכן מחוץ למסך, גודל חלון={{windowSize}}, גודל מסך={{displaySize}}, מיקום={{position}}" }
	},
	"dialog": {
		"hide-menu-enabled": {
		...
		"options": {
			...
			"submenu": {
				...
				"language": {
					...
					"label": "שפה",
					"submenu": { "to-help-translate": "רוצים לעזור לתרגם? לחץ כאן" }
				},
				...
				"visual-tweaks": {
					...
					"submenu": {
						"custom-window-title": { "prompt": { "placeholder": "לדוגמה: שולחן כתיבה אגסי" } },
						"like-buttons": {
							"default": "ברירת מחדל",
...
const plugins = {
	...
	"ambient-mode": {
		...
		"menu": {
			"blur-amount": {
				"label": "כמות טשטוש",
				"submenu": { "pixels": "{{blurAmount}} פיקסלים" }
			},
			"buffer": {
				"label": "חוצץ",
			...
			"quality": {
				"label": "אֵיכוּת",
				"submenu": { "pixels": "{{quality}} פיקסלים" }
			},
			"size": {
				"label": "גוֹדֶל",
			...
			"smoothness-transition": {
				"label": "מעבר חלק",
				"submenu": { "during": "במהלך {{interpolationTime}} שניות" }
			},
			"use-fullscreen": { "label": "שימוש במסך מלא" }
		},
		"name": "מצב אווירה"
	},
	"amuse": {
		"description": "מוסיף תמיכה ב-Arvoxify עבור הווידג'ט של Amuse המתנגן כעת על ידי 6K Labs",
		"name": "משעשע",
		"response": { "query": "שרת ה-API של Amuse פועל. קבל מידע על השיר באמצעות GET /query." }
	},
	"api-server": {
		...
		"menu": {
			"auth-strategy": {
				"label": "שיטת אימות",
				"submenu": {
					"auth-at-first": { "label": "לאמת בבקשה הראשונה" },
					"none": { "label": "ללא אימות" }
				}
			},
			"hostname": { "label": "שם שרת אחסון" },
			"port": { "label": "פורט" }
		},
	...
	"captions-selector": {
		"description": "בורר כתוביות עבור רצועות אודיו של Arvoxify",
		"menu": {
		...
		"templates": { "title": "פתח בחירת כתוביות" },
		"toast": {
	...
	"discord": {
		...
		"menu": {
			...
			"play-on-arvoxify": "הפעל ביוטיוב מיוזיק",
...
export { common, he_default as default, language, main, plugins };
