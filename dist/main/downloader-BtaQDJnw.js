import { a as __toCommonJS, i as __require, n as __esmMin, o as __toDynamicImportESM, r as __export, s as __toESM, t as __commonJSMin } from "./chunk-0rTXi_Jc.js";
import { t as require_is } from "./is-DL8kkJAd.js";
import { _ as deepmerge } from "./config-n3n9Pwqt.js";
...
import { a as registerCallback, i as getImage, n as SongInfoEvent, r as cleanupName, s as Mutex, t as MediaType } from "./song-info-BTWNuwrN.js";
import { a as MusicResponsiveListItemFlexColumn, i as MusicResponsiveListItem, n as Platform, o as BrowserMediaSession, r as streamToIterable, s as UniversalCache, t as Innertube } from "./node-uWJIOUoK.js";
...
var style_default = ".ytmd-menu-item{display:var(--ytmusic-menu-item_-_display);height:var(--ytmusic-menu-item_-_height);align-items:var(--ytmusic-menu-item_-_align-items);padding:var(--ytmusic-menu-item_-_padding);cursor:pointer}.ytmd-menu-item>.yt-simple-endpoint:hover{background-color:var(--ytmusic-menu-item-hover-background-color)}.ytmd-menu-item{flex:var(--ytmusic-menu-item-icon_-_flex);margin:var(--ytmusic-menu-item-icon_-_margin);fill:var(--ytmusic-menu-item-icon_-_fill);stroke:var(--iron-icon-stroke-color,none);width:var(--iron-icon-width,24px);height:var(--iron-icon-height,24px);animation:var(--iron-icon_-_animation)}";
...
function filenamify(string, options = {}) {
	...
	if (replacement.length > 0) {
		if (!(string[0] === ".") && string[0] === ".") string = replacement + string;
...
var require_ID3Definitions = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var FRAME_IDENTIFIERS = {
		v2: {
			album: "TAL",
			bpm: "TBP",
			composer: "TCM",
	...
	var FRAME_INTERNAL_IDENTIFIERS = Object.keys(FRAME_IDENTIFIERS).reduce((acc, versionKey) => {
		acc[versionKey] = Object.keys(FRAME_IDENTIFIERS[versionKey]).reduce((acc$1, tagKey) => {
			acc$1[FRAME_IDENTIFIERS[versionKey][tagKey]] = tagKey;
			return acc$1;
		}, {});
		return acc;
	}, {});
	var ID3_FRAME_OPTIONS = {
		"PIC": { multiple: false },
		"WCM": { multiple: true },
		"WAR": { multiple: true },
		"T___": { multiple: false },
	...
	var TagConstants = {
		...
		SynchronisedLyrics: { ContentType: {
			OTHER: 0,
			LYRICS: 1,
	...
	module.exports.ENCODINGS = ENCODINGS$1;
...
var require_utf7 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	"+".charCodeAt(0);
	var minusChar = "-".charCodeAt(0), andChar = "&".charCodeAt(0);
	...
	Utf7IMAPEncoder.prototype.write = function(str) {
		...
		for (var i$3 = 0; i$3 < str.length; i$3++) {
			...
			if (32 <= uChar && uChar <= 126) {
				...
				if (!inBase64) {
					buf[bufIdx++] = uChar;
					if (uChar === 38) buf[bufIdx++] = minusChar;
				}
			} else {
				if (!inBase64) {
					buf[bufIdx++] = andChar;
...
var require_dbcs_codec = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function DBCSCodec(codecOptions, iconv$2) {
		...
		if (typeof codecOptions.gb18030 === "function") {
			...
			for (var i$3 = 129; i$3 <= 254; i$3++) {
				...
				for (var j = 48; j <= 57; j++) {
					if (secondByteNode[j] === -1) secondByteNode[j] = NODE_START - commonThirdByteNodeIdx;
					else if (secondByteNode[j] > -1e3) throw new Error("gb18030 decode tables conflict at byte 2");
					var thirdByteNode = this.decodeTables[NODE_START - secondByteNode[j]];
					for (var k = 129; k <= 254; k++) {
						if (thirdByteNode[k] === -1) thirdByteNode[k] = NODE_START - commonFourthByteNodeIdx;
						else if (thirdByteNode[k] === -1e3 - commonFourthByteNodeIdx) continue;
						else if (thirdByteNode[k] > -1e3) throw new Error("gb18030 decode tables conflict at byte 3");
						var fourthByteNode = this.decodeTables[NODE_START - thirdByteNode[k]];
						for (var l = 48; l <= 57; l++) if (fourthByteNode[l] === -1) fourthByteNode[l] = GB18030_CODE;
					}
				}
			}
		}
	...
	DBCSCodec.prototype._getDecodeTrieNode = function(addr) {
		...
		for (var i$3 = bytes.length - 1; i$3 > 0; i$3--) {
			...
			} else if (val <= -1e3) node = this.decodeTables[NODE_START - val];
	...
	DBCSCodec.prototype._setEncodeChar = function(uCode, dbcsCode) {
		...
		var low = uCode & 255;
		if (bucket[low] <= -10) this.encodeTableSeq[SEQ_START - bucket[low]][DEF_CHAR] = dbcsCode;
		else if (bucket[low] == -1) bucket[low] = dbcsCode;
	};
	DBCSCodec.prototype._setEncodeSequence = function(seq, dbcsCode) {
		...
		var low = uCode & 255;
		var node;
		if (bucket[low] <= -10) node = this.encodeTableSeq[SEQ_START - bucket[low]];
		else {
			node = {};
			if (bucket[low] !== -1) node[DEF_CHAR] = bucket[low];
	...
	DBCSEncoder.prototype.write = function(str) {
		...
		while (true) {
			...
			if (dbcsCode === -1) dbcsCode = this.defaultCharSingleByte;
...
var shiftjis_exports = {};
__export(shiftjis_exports, { default: () => shiftjis_default });
...
var eucjp_exports = {};
__export(eucjp_exports, { default: () => eucjp_default });
var eucjp_default;
...
var cp936_exports = {};
__export(cp936_exports, { default: () => cp936_default });
var cp936_default;
...
var gbk_added_exports = {};
__export(gbk_added_exports, { default: () => gbk_added_default });
...
var gb18030_ranges_exports = {};
__export(gb18030_ranges_exports, {
...
var cp949_exports = {};
__export(cp949_exports, { default: () => cp949_default });
var cp949_default;
...
var cp950_exports = {};
__export(cp950_exports, { default: () => cp950_default });
var cp950_default;
...
var big5_added_exports = {};
__export(big5_added_exports, { default: () => big5_added_default });
...
var require_ID3Util = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var iconv = require_lib$1();
	var ID3Definitions$3 = require_ID3Definitions();
	var ENCODINGS = [
		"ISO-8859-1",
		"UTF-16",
		"UTF-16BE",
	...
	module.exports.splitNullTerminatedBuffer = function(buffer$1, encodingByte = 0) {
		const charSize = [1, 2].includes(encodingByte) ? 2 : 1;
		for (let pos = 0; pos + charSize - 1 < buffer$1.length; pos += charSize) if (buffer$1.readUIntBE(pos, charSize) === 0) return new this.SplitBuffer(buffer$1.subarray(0, pos), buffer$1.subarray(pos + charSize));
		return new this.SplitBuffer(null, buffer$1.subarray(0));
	};
	module.exports.terminationBuffer = function(encodingByte = 0) {
		if (encodingByte === 1 || encodingByte === 2) return Buffer.alloc(2, 0);
		return Buffer.alloc(1, 0);
	};
	module.exports.encodingFromStringOrByte = function(encoding) {
		if (ENCODINGS.indexOf(encoding) !== -1) return encoding;
		else if (encoding > -1 && encoding < ENCODINGS.length) encoding = ENCODINGS[encoding];
		else encoding = ENCODINGS[0];
		return encoding;
	};
	...
	module.exports.bufferToDecodedString = function(buffer$1, encodingByte) {
		return iconv.decode(buffer$1, this.encodingFromStringOrByte(encodingByte)).replace(/\0/g, "");
	};
	module.exports.getSpecOptions = function(frameIdentifier) {
		if (ID3Definitions$3.ID3_FRAME_OPTIONS[frameIdentifier]) return ID3Definitions$3.ID3_FRAME_OPTIONS[frameIdentifier];
		return {};
	};
	module.exports.isValidID3Header = function(buffer$1) {
		if (buffer$1.length < 10) return false;
		if (buffer$1.readUIntBE(0, 3) !== 4801587) return false;
		if ([
			2,
			3,
			4
		].indexOf(buffer$1[3]) === -1 || buffer$1[4] !== 0) return false;
		return this.isValidEncodedSize(buffer$1.slice(6, 10));
	};
	module.exports.getFramePosition = function(buffer$1) {
		let framePosition = -1;
		let frameHeaderValid = false;
		do {
			framePosition = buffer$1.indexOf("ID3", framePosition + 1);
			if (framePosition !== -1) frameHeaderValid = this.isValidID3Header(buffer$1.slice(framePosition, framePosition + 10));
		} while (framePosition !== -1 && !frameHeaderValid);
		if (!frameHeaderValid) return -1;
	...
	module.exports.encodeSize = function(size) {
		const byte_3 = size & 127;
		const byte_2 = size >> 7 & 127;
		const byte_1 = size >> 14 & 127;
		const byte_0 = size >> 21 & 127;
		return Buffer.from([
			byte_0,
			byte_1,
			byte_2,
	...
	module.exports.getFrameSize = function(buffer$1, decode, ID3Version) {
		let decodeBytes;
		if (ID3Version > 2) decodeBytes = [
			buffer$1[4],
			buffer$1[5],
			buffer$1[6],
			buffer$1[7]
		...
		if (decode) return this.decodeSize(Buffer.from(decodeBytes));
		else return Buffer.from(decodeBytes).readUIntBE(0, decodeBytes.length);
	};
	module.exports.parseTagHeaderFlags = function(header) {
		if (!(header instanceof Buffer && header.length >= 10)) return {};
	...
	module.exports.processUnsynchronisedBuffer = function(buffer$1) {
		const newDataArr = [];
		if (buffer$1.length > 0) newDataArr.push(buffer$1[0]);
		for (let i$3 = 1; i$3 < buffer$1.length; i$3++) {
			if (buffer$1[i$3 - 1] === 255 && buffer$1[i$3] === 0) continue;
			newDataArr.push(buffer$1[i$3]);
		}
		return Buffer.from(newDataArr);
	};
	module.exports.getPictureMimeTypeFromBuffer = function(pictureBuffer) {
		if (pictureBuffer.length > 3 && pictureBuffer.compare(Buffer.from([
			255,
			216,
			255
		]), 0, 3, 0, 3) === 0) return "image/jpeg";
		else if (pictureBuffer > 8 && pictureBuffer.compare(Buffer.from([
			137,
			80,
			78,
			71,
			...
			26,
			10
		]), 0, 8, 0, 8) === 0) return "image/png";
		else return null;
	};
}));
var require_ID3FrameBuilder = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = ID3FrameBuilder$1;
	var ID3Util$4 = require_ID3Util();
	function ID3FrameBuilder$1(identifier) {
		this._identifier = identifier;
		this._buffer = Buffer.alloc(0);
	}
	ID3FrameBuilder$1.prototype.appendStaticValue = function(value, size, encoding = 0) {
		const convertedValue = convertValue$1(value, encoding);
		this._buffer = Buffer.concat([this._buffer, staticValueToBuffer(convertedValue, size)]);
		return this;
	};
	ID3FrameBuilder$1.prototype.appendStaticNumber = function(value, size) {
		if (!Number.isInteger(value)) throw new RangeError("An integer value is expected");
	...
	ID3FrameBuilder$1.prototype.appendNullTerminatedValue = function(value, encoding = 0) {
		value = value || "";
		const convertedValue = convertValue$1(value, encoding);
		this._buffer = Buffer.concat([this._buffer, nullTerminatedValueToBuffer(convertedValue, encoding)]);
		return this;
	};
	ID3FrameBuilder$1.prototype.getBuffer = function() {
		const header = Buffer.alloc(10);
		header.write(this._identifier, 0);
		header.writeUInt32BE(this._buffer.length, 4);
		return Buffer.concat([header, this._buffer]);
	};
	function convertValue$1(value, encoding = 0) {
		if (value instanceof Buffer) return value;
		if (Number.isInteger(value)) return ID3Util$4.stringToEncodedBuffer(value.toString(), encoding);
		if (typeof value === "string" || value instanceof String) return ID3Util$4.stringToEncodedBuffer(value, encoding);
		return Buffer.alloc(0);
	}
	function staticValueToBuffer(buffer$1, size) {
		if (!(buffer$1 instanceof Buffer)) return Buffer.alloc(0);
		if (size && buffer$1.length < size) return Buffer.concat([Buffer.alloc(size - buffer$1.length, 0), buffer$1]);
		return buffer$1;
	}
	function nullTerminatedValueToBuffer(buffer$1, encoding) {
		return Buffer.concat([buffer$1, ID3Util$4.terminationBuffer(encoding)]);
	}
}));
var require_ID3FrameReader = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = ID3FrameReader$1;
	var ID3Util$3 = require_ID3Util();
	function ID3FrameReader$1(buffer$1, encodingBytePosition, consumeEncodingByte = true) {
		if (!buffer$1 || !(buffer$1 instanceof Buffer)) buffer$1 = Buffer.alloc(0);
		if (Number.isInteger(encodingBytePosition)) {
			this._encoding = buffer$1[encodingBytePosition] ? buffer$1[encodingBytePosition] : 0;
			if (consumeEncodingByte) buffer$1 = encodingBytePosition === 0 ? buffer$1.slice(1) : Buffer.concat([buffer$1.slice(0, encodingBytePosition), buffer$1.slice(encodingBytePosition)]);
		} else this._encoding = 0;
		this._splitBuffer = new ID3Util$3.SplitBuffer(null, buffer$1.slice(0));
	}
	ID3FrameReader$1.prototype.consumeStaticValue = function(dataType, size, encoding = this._encoding) {
		return this._consumeByFunction(() => staticValueFromBuffer(this._splitBuffer.remainder, size), dataType, encoding);
	};
	ID3FrameReader$1.prototype.consumeNullTerminatedValue = function(dataType, encoding = this._encoding) {
		return this._consumeByFunction(() => nullTerminatedValueFromBuffer(this._splitBuffer.remainder, encoding), dataType, encoding);
	};
	ID3FrameReader$1.prototype._consumeByFunction = function(fn, dataType, encoding) {
		if (!this._splitBuffer.remainder || this._splitBuffer.remainder.length === 0) return;
		this._splitBuffer = fn();
		if (dataType) return convertValue(this._splitBuffer.value, dataType, encoding);
		return this._splitBuffer.value;
	};
	function convertValue(buffer$1, dataType, encoding = 0) {
		if (!buffer$1) return;
		if (!(buffer$1 instanceof Buffer)) return buffer$1;
		if (buffer$1.length === 0) return;
		if (dataType === "number") return parseInt(buffer$1.toString("hex"), 16);
		if (dataType === "string") return ID3Util$3.bufferToDecodedString(buffer$1, encoding);
		return buffer$1;
	}
	function staticValueFromBuffer(buffer$1, size) {
		if (size === void 0 || size === null) size = buffer$1.length;
		if (buffer$1.length > size) return new ID3Util$3.SplitBuffer(buffer$1.slice(0, size), buffer$1.slice(size));
		return new ID3Util$3.SplitBuffer(buffer$1.slice(0), null);
	}
	function nullTerminatedValueFromBuffer(buffer$1, encoding = 0) {
		return ID3Util$3.splitNullTerminatedBuffer(buffer$1, encoding);
	}
}));
...
var require_ID3Frames = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var fs$2 = __require("fs");
	var ID3FrameBuilder = require_ID3FrameBuilder();
	var ID3FrameReader = require_ID3FrameReader();
	var ID3Definitions$2 = require_ID3Definitions();
	var ID3Util$2 = require_ID3Util();
	var ID3Helpers$1 = require_ID3Helpers();
	var { isString: isString$1 } = require_util();
	module.exports.GENERIC_TEXT = {
		create: (frameIdentifier, data) => {
			if (!frameIdentifier || !data) return null;
			return new ID3FrameBuilder(frameIdentifier).appendStaticNumber(1, 1).appendStaticValue(data, null, 1).getBuffer();
		},
		read: (buffer$1) => {
			return new ID3FrameReader(buffer$1, 0).consumeStaticValue("string");
		}
	};
	module.exports.GENERIC_URL = {
		...
		read: (buffer$1) => {
			return new ID3FrameReader(buffer$1).consumeStaticValue("string");
		}
	};
	module.exports.APIC = {
		create: (data) => {
			try {
				if (data instanceof Buffer) data = { imageBuffer: Buffer.from(data) };
				else if (isString$1(data)) data = { imageBuffer: fs$2.readFileSync(data) };
				else if (!data.imageBuffer) return Buffer.alloc(0);
				let mime_type = data.mime;
				if (!mime_type) mime_type = ID3Util$2.getPictureMimeTypeFromBuffer(data.imageBuffer);
				const TagConstants$1 = ID3Definitions$2.TagConstants.AttachedPicture;
				const pictureType = data.type || {};
				const pictureTypeId = pictureType.id === void 0 ? TagConstants$1.PictureType.FRONT_COVER : pictureType.id;
				const { description = "" } = data;
				const encoding = description ? 1 : 0;
				return new ID3FrameBuilder("APIC").appendStaticNumber(encoding, 1).appendNullTerminatedValue(mime_type).appendStaticNumber(pictureTypeId, 1).appendNullTerminatedValue(description, encoding).appendStaticValue(data.imageBuffer).getBuffer();
			} catch (error) {
				return error;
			}
		},
		read: (buffer$1, version) => {
			const reader = new ID3FrameReader(buffer$1, 0);
			let mime;
			if (version === 2) mime = reader.consumeStaticValue("string", 3, 0);
			else mime = reader.consumeNullTerminatedValue("string", 0);
			...
			return {
				...
				type: {
					id: typeId,
					name: ID3Definitions$2.APIC_TYPES[typeId]
				},
				description,
				imageBuffer
			};
	...
	module.exports.COMM = {
		create: (data) => {
			...
			return new ID3FrameBuilder("COMM").appendStaticNumber(1, 1).appendStaticValue(data.language).appendNullTerminatedValue(data.shortText, 1).appendStaticValue(data.text, null, 1).getBuffer();
		},
		read: (buffer$1) => {
			const reader = new ID3FrameReader(buffer$1, 0);
			return {
				language: reader.consumeStaticValue("string", 3, 0),
	...
	module.exports.USLT = {
		create: (data) => {
			data = data || {};
			if (isString$1(data)) data = { text: data };
			if (!data.text) return null;
			return new ID3FrameBuilder("USLT").appendStaticNumber(1, 1).appendStaticValue(data.language).appendNullTerminatedValue(data.shortText, 1).appendStaticValue(data.text, null, 1).getBuffer();
		},
		read: (buffer$1) => {
			const reader = new ID3FrameReader(buffer$1, 0);
			return {
	...
	module.exports.SYLT = {
		create: (data) => {
			...
			return Buffer.concat(data.map((lycics) => {
				const frameBuilder = new ID3FrameBuilder("SYLT").appendStaticNumber(encoding, 1).appendStaticValue(lycics.language, 3).appendStaticNumber(lycics.timeStampFormat, 1).appendStaticNumber(lycics.contentType, 1).appendNullTerminatedValue(lycics.shortText, encoding);
			...
			return {
				...
				synchronisedText: Array.from((function* () {
					while (true) {
						...
						if (text === void 0 || timeStamp === void 0) break;
						yield {
							text,
							timeStamp
						};
	...
	module.exports.TXXX = {
		create: (data) => {
			if (!(data instanceof Array)) data = [data];
			return Buffer.concat(data.map((udt) => new ID3FrameBuilder("TXXX").appendStaticNumber(1, 1).appendNullTerminatedValue(udt.description, 1).appendStaticValue(udt.value, null, 1).getBuffer()));
		},
		read: (buffer$1) => {
			const reader = new ID3FrameReader(buffer$1, 0);
			return {
	...
	module.exports.POPM = {
		...
		read: (buffer$1) => {
			const reader = new ID3FrameReader(buffer$1);
			return {
	...
	module.exports.UFID = {
		create: (data) => {
			if (!(data instanceof Array)) data = [data];
			return Buffer.concat(data.map((ufid) => new ID3FrameBuilder("UFID").appendNullTerminatedValue(ufid.ownerIdentifier).appendStaticValue(ufid.identifier instanceof Buffer ? ufid.identifier : Buffer.from(ufid.identifier, "utf8")).getBuffer()));
		},
		read: (buffer$1) => {
			const reader = new ID3FrameReader(buffer$1);
			return {
	...
	module.exports.CHAP = {
		create: (data) => {
			...
			return Buffer.concat(data.map((chap) => {
				if (!chap || !chap.elementID || typeof chap.startTimeMs === "undefined" || !chap.endTimeMs) return null;
				return new ID3FrameBuilder("CHAP").appendNullTerminatedValue(chap.elementID).appendStaticNumber(chap.startTimeMs, 4).appendStaticNumber(chap.endTimeMs, 4).appendStaticNumber(chap.startOffsetBytes ? chap.startOffsetBytes : 4294967295, 4).appendStaticNumber(chap.endOffsetBytes ? chap.endOffsetBytes : 4294967295, 4).appendStaticValue(ID3Helpers$1.createBufferFromTags(chap.tags)).getBuffer();
		...
		read: (buffer$1) => {
			...
			const chap = {
				...
				tags: ID3Helpers$1.getTagsFromID3Body(reader.consumeStaticValue())
			};
			if (chap.startOffsetBytes === 4294967295) delete chap.startOffsetBytes;
			if (chap.endOffsetBytes === 4294967295) delete chap.endOffsetBytes;
			return chap;
		}
	};
	module.exports.CTOC = {
		create: (data) => {
			...
			return Buffer.concat(data.map((toc, index) => {
				if (!toc || !toc.elementID) return null;
				if (!(toc.elements instanceof Array)) toc.elements = [];
				const ctocFlags = Buffer.alloc(1, 0);
				if (index === 0) ctocFlags[0] += 2;
				if (toc.isOrdered) ctocFlags[0] += 1;
				...
				if (toc.tags) builder.appendStaticValue(ID3Helpers$1.createBufferFromTags(toc.tags));
		...
		read: (buffer$1) => {
			...
			const elements = [];
			for (let i$3 = 0; i$3 < entries; i$3++) elements.push(reader.consumeNullTerminatedValue("string"));
			const tags = ID3Helpers$1.getTagsFromID3Body(reader.consumeStaticValue());
			return {
				elementID,
				isOrdered: !!(flags & true),
				elements,
				tags
			};
		}
	...
	module.exports.WXXX = {
		create: (data) => {
			...
			return Buffer.concat(data.map((udu) => {
				return new ID3FrameBuilder("WXXX").appendStaticNumber(1, 1).appendNullTerminatedValue(udu.description, 1).appendStaticValue(udu.url, null).getBuffer();
			}));
		},
		read: (buffer$1) => {
			const reader = new ID3FrameReader(buffer$1, 0);
			return {
				description: reader.consumeNullTerminatedValue("string"),
				url: reader.consumeStaticValue("string", null, 0)
			};
		}
	};
	module.exports.ETCO = {
			...
			return {
				...
				keyEvents: Array.from((function* () {
					while (true) {
						...
						const timeStamp = reader.consumeStaticValue("number", 4);
						if (type === void 0 || timeStamp === void 0) break;
						yield {
							type,
							timeStamp
						};
	...
	module.exports.COMR = {
		create: (data) => {
			...
			return Buffer.concat(data.map((comr) => {
				...
				builder.appendStaticNumber(1, 1);
				const priceString = Object.entries(prices).map((price) => {
					return price[0].substring(0, 3) + price[1].toString();
				}).join("/");
				builder.appendNullTerminatedValue(priceString, 0);
				builder.appendStaticValue(comr.validUntil.year.toString().padStart(4, "0").substring(0, 4) + comr.validUntil.month.toString().padStart(2, "0").substring(0, 2) + comr.validUntil.day.toString().padStart(2, "0").substring(0, 2), 8, 0);
				builder.appendNullTerminatedValue(comr.contactUrl, 0);
				builder.appendStaticNumber(comr.receivedAs, 1);
				builder.appendNullTerminatedValue(comr.nameOfSeller, 1);
				builder.appendNullTerminatedValue(comr.description, 1);
				if (comr.sellerLogo) {
					let picture = comr.sellerLogo.picture;
					if (typeof comr.sellerLogo.picture === "string" || comr.sellerLogo.picture instanceof String) picture = fs$2.readFileSync(comr.sellerLogo.picture);
					let mimeType = comr.sellerLogo.mimeType || ID3Util$2.getPictureMimeTypeFromBuffer(picture);
					if (mimeType !== "image/png" && "image/jpeg") mimeType = "image/";
					builder.appendNullTerminatedValue(mimeType ? mimeType : "", 0);
		...
		read: (buffer$1) => {
			const reader = new ID3FrameReader(buffer$1, 0);
			const tag = {};
			const priceStrings = reader.consumeNullTerminatedValue("string", 0).split("/").filter((price) => price.length > 3);
			tag.prices = {};
			for (const price of priceStrings) tag.prices[price.substring(0, 3)] = price.substring(3);
			const validUntilString = reader.consumeStaticValue("string", 8, 0);
			tag.validUntil = {
				year: 0,
				month: 0,
				day: 0
			...
			tag.contactUrl = reader.consumeNullTerminatedValue("string", 0);
			...
			const mimeType = reader.consumeNullTerminatedValue("string", 0);
			const picture = reader.consumeStaticValue("buffer");
			if (picture && picture.length > 0) tag.sellerLogo = {
				mimeType,
				picture
			};
			return tag;
	...
	module.exports.GEOB = {
		create: (data) => {
			if (!(data instanceof Array)) data = [data];
			data.forEach((item) => {
				if (item.encapsulatedObject == null) throw new Error("encapsulatedObject is required in GEOB frames");
				if (item.contentDescription == null) throw new Error("contentDescription is required for GEOB frames");
			});
			const encoding = 1;
			return Buffer.concat(data.map((geob) => {
				return new ID3FrameBuilder("GEOB").appendStaticNumber(encoding).appendNullTerminatedValue(geob.mimeType ? geob.mimeType : "").appendNullTerminatedValue(geob.filename ? geob.filename : "", encoding).appendNullTerminatedValue(geob.contentDescription, encoding).appendStaticValue(geob.encapsulatedObject).getBuffer();
			}));
		},
		read: (buffer$1) => {
			const reader = new ID3FrameReader(buffer$1, 0);
...
var require_ID3Helpers = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var zlib = __require("zlib");
	var ID3Definitions$1 = require_ID3Definitions();
	var ID3Frames = require_ID3Frames();
	var ID3Util$1 = require_ID3Util();
	function createBuffersFromTags(tags) {
		const frames = [];
		if (!tags) return frames;
		const rawObject = Object.keys(tags).reduce((acc, val) => {
			if (ID3Definitions$1.FRAME_IDENTIFIERS.v3[val] !== void 0) acc[ID3Definitions$1.FRAME_IDENTIFIERS.v3[val]] = tags[val];
			else if (ID3Definitions$1.FRAME_IDENTIFIERS.v4[val] !== void 0) acc[ID3Definitions$1.FRAME_IDENTIFIERS.v4[val]] = tags[val];
			else acc[val] = tags[val];
			return acc;
		}, {});
		Object.keys(rawObject).forEach((frameIdentifier) => {
			let frame;
			if (frameIdentifier.length !== 4) return;
			if (ID3Frames[frameIdentifier] !== void 0) frame = ID3Frames[frameIdentifier].create(rawObject[frameIdentifier], 3);
			else if (frameIdentifier.startsWith("T")) frame = ID3Frames.GENERIC_TEXT.create(frameIdentifier, rawObject[frameIdentifier], 3);
			else if (frameIdentifier.startsWith("W")) if (ID3Util$1.getSpecOptions(frameIdentifier, 3).multiple && rawObject[frameIdentifier] instanceof Array && rawObject[frameIdentifier].length > 0) {
				frame = Buffer.alloc(0);
				for (const url of [...new Set(rawObject[frameIdentifier])]) frame = Buffer.concat([frame, ID3Frames.GENERIC_URL.create(frameIdentifier, url, 3)]);
			} else frame = ID3Frames.GENERIC_URL.create(frameIdentifier, rawObject[frameIdentifier], 3);
			if (frame && frame instanceof Buffer) frames.push(frame);
		});
		return frames;
	}
	...
	module.exports.getTagsFromBuffer = function(filebuffer, options) {
		const framePosition = ID3Util$1.getFramePosition(filebuffer);
		if (framePosition === -1) return getTagsFromFrames([], 3, options);
		const frameSize = ID3Util$1.decodeSize(filebuffer.slice(framePosition + 6, framePosition + 10)) + 10;
		const ID3Frame = Buffer.alloc(frameSize + 1);
		filebuffer.copy(ID3Frame, 0, framePosition);
		const ID3Version = ID3Frame[3];
		const tagFlags = ID3Util$1.parseTagHeaderFlags(ID3Frame);
		let extendedHeaderOffset = 0;
		if (tagFlags.extendedHeader) {
			if (ID3Version === 3) extendedHeaderOffset = 4 + filebuffer.readUInt32BE(10);
			else if (ID3Version === 4) extendedHeaderOffset = ID3Util$1.decodeSize(filebuffer.slice(10, 14));
		}
	...
	function isFrameDiscardedByOptions(frameIdentifier, options) {
		if (options.exclude instanceof Array && options.exclude.includes(frameIdentifier)) return true;
	...
	function getFramesFromID3Body(ID3TagBody, ID3Version, options = {}) {
		...
		const frames = [];
		if (!ID3TagBody || !(ID3TagBody instanceof Buffer)) return frames;
		const frameIdentifierSize = ID3Version === 2 ? 3 : 4;
		const frameHeaderSize = ID3Version === 2 ? 6 : 10;
		while (currentPosition < ID3TagBody.length && ID3TagBody[currentPosition] !== 0) {
			...
			const frameBodySize = ID3Util$1.getFrameSize(frameHeader, decodeSize, ID3Version);
			if (isFrameDiscardedByOptions(frameIdentifier, options)) {
				currentPosition += frameBodySize + frameHeaderSize;
				continue;
			}
			if (frameBodySize + frameHeaderSize > ID3TagBody.length - currentPosition) break;
			const frameHeaderFlags = ID3Util$1.parseFrameHeaderFlags(frameHeader, ID3Version);
			...
			const frame = {
				...
				body: frameHeaderFlags.unsynchronisation ? ID3Util$1.processUnsynchronisedBuffer(frameBody) : frameBody
			};
			if (frameHeaderFlags.dataLengthIndicator) frame.dataLengthIndicator = ID3TagBody.readInt32BE(currentPosition + frameHeaderSize);
			frames.push(frame);
			currentPosition += frameBodySize + frameHeaderSize;
		}
		return frames;
	...
	function decompressFrame(frame) {
		if (frame.body.length < 5 || frame.dataLengthIndicator === void 0) return null;
		...
		if (decompressedBody.length !== frame.dataLengthIndicator) return null;
	...
	function getTagsFromFrames(frames, ID3Version, options = {}) {
		...
		frames.forEach((frame) => {
			...
			let identifier;
			if (ID3Version === 2) {
				frameIdentifier = ID3Definitions$1.FRAME_IDENTIFIERS.v3[ID3Definitions$1.FRAME_INTERNAL_IDENTIFIERS.v2[frame.name]];
				identifier = ID3Definitions$1.FRAME_INTERNAL_IDENTIFIERS.v2[frame.name];
			} else if (ID3Version === 3 || ID3Version === 4) {
				frameIdentifier = frame.name;
				identifier = ID3Definitions$1.FRAME_INTERNAL_IDENTIFIERS.v3[frame.name] || ID3Definitions$1.FRAME_INTERNAL_IDENTIFIERS.v4[frame.name];
			}
			if (!frameIdentifier || !identifier || frame.flags.encryption) return;
			if (frame.flags.compression) {
				const decompressedBody = decompressFrame(frame);
				if (!decompressedBody) return;
				frame.body = decompressedBody;
			}
			let decoded;
			if (ID3Frames[frameIdentifier]) decoded = ID3Frames[frameIdentifier].read(frame.body, ID3Version);
			else if (frameIdentifier.startsWith("T")) decoded = ID3Frames.GENERIC_TEXT.read(frame.body, ID3Version);
			else if (frameIdentifier.startsWith("W")) decoded = ID3Frames.GENERIC_URL.read(frame.body, ID3Version);
			if (!decoded) return;
			if (ID3Util$1.getSpecOptions(frameIdentifier, ID3Version).multiple) {
				if (!options.onlyRaw) {
					if (!tags[identifier]) tags[identifier] = [];
					tags[identifier].push(decoded);
				}
				if (!options.noRaw) {
					if (!raw[frameIdentifier]) raw[frameIdentifier] = [];
					raw[frameIdentifier].push(decoded);
				}
			} else {
				if (!options.onlyRaw) tags[identifier] = decoded;
				if (!options.noRaw) raw[frameIdentifier] = decoded;
			}
		});
...
var require_node_id3 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var fs$1 = __require("fs");
	var ID3Definitions = require_ID3Definitions();
	var ID3Util = require_ID3Util();
	var ID3Helpers = require_ID3Helpers();
	var { isFunction, isString } = require_util();
	function removeTagsFromBuffer(data) {
		...
		if (!ID3Util.isValidEncodedSize(encodedSize)) return false;
	...
	function writeInBuffer(tags, buffer$1) {
		buffer$1 = removeTagsFromBuffer(buffer$1) || buffer$1;
		return Buffer.concat([tags, buffer$1]);
	}
	function writeAsync(tags, filebuffer, fn) {
		if (isString(filebuffer)) try {
			fs$1.readFile(filebuffer, (error, data) => {
				if (error) {
					fn(error);
					return;
				}
				const newData = writeInBuffer(tags, data);
				fs$1.writeFile(filebuffer, newData, "binary", (error$1) => {
					fn(error$1);
				});
			});
		} catch (error) {
		...
		else fn(null, writeInBuffer(tags, filebuffer));
	}
	function writeSync(tags, filebuffer) {
		if (isString(filebuffer)) try {
			const data = fs$1.readFileSync(filebuffer);
			const newData = writeInBuffer(tags, data);
			fs$1.writeFileSync(filebuffer, newData, "binary");
			return true;
		} catch (error) {
			return error;
		}
	...
	function write(tags, filebuffer, fn) {
		const completeTags = create$1(tags);
		if (isFunction(fn)) return writeAsync(completeTags, filebuffer, fn);
		return writeSync(completeTags, filebuffer);
	}
	function create$1(tags, fn) {
		const frames = ID3Helpers.createBufferFromTags(tags);
		const header = Buffer.alloc(10);
		header.fill(0);
		header.write("ID3", 0);
		header.writeUInt16BE(768, 3);
	...
	function readSync(filebuffer, options) {
		if (isString(filebuffer)) filebuffer = fs$1.readFileSync(filebuffer);
		return ID3Helpers.getTagsFromBuffer(filebuffer, options);
	}
	function readAsync(filebuffer, options, fn) {
		if (isString(filebuffer)) fs$1.readFile(filebuffer, (error, data) => {
			if (error) fn(error, null);
			else fn(null, ID3Helpers.getTagsFromBuffer(data, options));
		});
		else fn(null, ID3Helpers.getTagsFromBuffer(filebuffer, options));
	}
	function read(filebuffer, options, fn) {
		...
		if (isFunction(fn)) return readAsync(filebuffer, options, fn);
	...
	function update(tags, filebuffer, options, fn) {
		...
		const rawTags = Object.keys(tags).reduce((acc, val) => {
			if (ID3Definitions.FRAME_IDENTIFIERS.v3[val] !== void 0) acc[ID3Definitions.FRAME_IDENTIFIERS.v3[val]] = tags[val];
			else acc[val] = tags[val];
			return acc;
		}, {});
		const updateFn = (currentTags) => {
			...
			Object.keys(rawTags).map((frameIdentifier) => {
				const options$1 = ID3Util.getSpecOptions(frameIdentifier, 3);
				const cCompare = {};
				if (options$1.multiple && currentTags[frameIdentifier] && rawTags[frameIdentifier]) {
					if (options$1.updateCompareKey) currentTags[frameIdentifier].forEach((cTag, index) => {
						cCompare[cTag[options$1.updateCompareKey]] = index;
					});
					if (!(rawTags[frameIdentifier] instanceof Array)) rawTags[frameIdentifier] = [rawTags[frameIdentifier]];
					rawTags[frameIdentifier].forEach((rTag) => {
						const comparison = cCompare[rTag[options$1.updateCompareKey]];
						if (comparison !== void 0) currentTags[frameIdentifier][comparison] = rTag;
						else currentTags[frameIdentifier].push(rTag);
					});
				} else currentTags[frameIdentifier] = rawTags[frameIdentifier];
			});
			return currentTags;
		};
		if (isFunction(fn)) return write(updateFn(read(filebuffer, options)), filebuffer, fn);
		return write(updateFn(read(filebuffer, options)), filebuffer);
	}
	function removeTagsSync(filepath) {
		let data;
		try {
			data = fs$1.readFileSync(filepath);
		} catch (error) {
			return error;
		}
		...
		try {
			fs$1.writeFileSync(filepath, newData, "binary");
		} catch (error) {
			return error;
		}
		return true;
	...
	function removeTagsAsync(filepath, fn) {
		fs$1.readFile(filepath, (error, data) => {
			if (error) {
				fn(error);
				return;
			}
			...
			fs$1.writeFile(filepath, newData, "binary", (error$1) => {
				if (error$1) fn(error$1);
				else fn(null);
			});
		});
	}
	function removeTags(filepath, fn) {
		if (isFunction(fn)) return removeTagsAsync(filepath, fn);
		return removeTagsSync(filepath);
	}
	function makeSwapParameters(fn) {
		return (a, b) => fn(b, a);
	}
	function makePromise(fn, reorderParameters = (fn$1) => (a, b) => fn$1(a, b)) {
	...
	module.exports = {
		...
		Promise: {
			create: (tags) => makePromise(create$1.bind(null, tags), makeSwapParameters),
...
function base64ToU8(base64) {
	let base64Mod;
	if (base64urlCharRegex.test(base64)) base64Mod = base64.replace(base64urlCharRegex, function(match) {
		return base64urlToBase64Map[match];
	});
	else base64Mod = base64;
...
function parseChallengeData(rawData) {
	...
	} else if (rawData.length && typeof rawData[0] === "object") challengeData = rawData[0];
...
function descramble(scrambledChallenge) {
	const buffer$1 = base64ToU8(scrambledChallenge);
	if (buffer$1.length) return new TextDecoder().decode(buffer$1.map((b) => b + 97));
}
var BotGuardClient = class BotGuardClient {
	...
	async snapshot(args, timeout = 3e3) {
		return await Promise.race([new Promise(async (resolve, reject) => {
	...
	async passEvent(args, timeout = this.defaultTimeout) {
		return await Promise.race([(async () => {
			const vmFunctions = await this.deferredVmFunctions.promise;
			if (!vmFunctions.passEventFunction) throw new BGError("PASS_EVENT", "Pass event function not found");
	...
	async checkCamera(args, timeout = this.defaultTimeout) {
		return await Promise.race([(async () => {
			const vmFunctions = await this.deferredVmFunctions.promise;
			if (!vmFunctions.checkCameraFunction) throw new BGError("CHECK_CAMERA", "Check camera function not found");
	...
	async shutdown(timeout = this.defaultTimeout) {
		return await Promise.race([(async () => {
			const vmFunctions = await this.deferredVmFunctions.promise;
			if (!vmFunctions.shutdownFunction) throw new BGError("SHUTDOWN", "Shutdown function not found");
			vmFunctions.shutdownFunction();
		})(), new Promise((_, reject) => setTimeout(() => reject(new BGError("TIMEOUT", "VM operation timed out")), timeout))]);
	}
	async snapshotSynchronous(args) {
		if (!this.syncSnapshotFunction) throw new BGError("SYNC_SNAPSHOT", "Synchronous snapshot function not found");
...
async function generate(args) {
	...
	const [integrityToken, estimatedTtlSecs, mintRefreshThreshold, websafeFallbackToken] = await (await bgConfig.fetch(buildURL("GenerateIT", bgConfig.useYouTubeAPI), {
		method: "POST",
		headers: getHeaders(),
		body: JSON.stringify(payload)
	})).json();
	...
	return {
		poToken: await (await WebPoMinter.create(integrityTokenData, webPoSignalOutput)).mintAsWebsafeString(identifier),
		integrityTokenData
	};
}
var import_lib = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	(() => {
		var i$3 = {
			d: (e$1, t$2) => {
				for (var n$1 in t$2) i$3.o(t$2, n$1) && !i$3.o(e$1, n$1) && Object.defineProperty(e$1, n$1, {
					enumerable: !0,
					get: t$2[n$1]
				});
			},
			o: (i$4, e$1) => Object.prototype.hasOwnProperty.call(i$4, e$1),
			r: (i$4) => {
				"undefined" != typeof Symbol && Symbol.toStringTag && Object.defineProperty(i$4, Symbol.toStringTag, { value: "Module" }), Object.defineProperty(i$4, "__esModule", { value: !0 });
			}
		}, e = {};
		i$3.r(e), i$3.d(e, {
			AsyncLazy: () => s,
		...
		class t$1 {
			constructor() {
				this.promise = new Promise(((i$4, e$1) => {
					this.res = i$4, this.rej = e$1;
				}));
			}
			unblock(i$4) {
				if (null == this.res) throw new Error("Can't unblock");
				this.res(i$4);
			}
			throw(i$4) {
				if (null == this.rej) throw new Error("Can't unblock");
				this.rej(i$4);
			}
		}
		var n = function(i$4, e$1, t$2, n$1) {
			return new (t$2 || (t$2 = Promise))((function(r$1, s$1) {
				function u$1(i$5) {
					try {
						o$1(n$1.next(i$5));
					} catch (i$6) {
				...
				function o$1(i$5) {
					var e$2;
					i$5.done ? r$1(i$5.value) : (e$2 = i$5.value, e$2 instanceof t$2 ? e$2 : new t$2((function(i$6) {
						i$6(e$2);
					}))).then(u$1, l$1);
				}
				o$1((n$1 = n$1.apply(i$4, e$1 || [])).next());
			}));
		};
		class r {
			constructor(i$4) {
				this._value = null, this._initialized = !1, this._initializer = i$4;
			}
			get initializer() {
		...
		class s extends r {
			...
			get(...i$4) {
				return n(this, void 0, void 0, (function* () {
					const e$1 = new t$1();
					for (yield Promise.all(this.queue), this.queue.push(e$1.promise), this._initialized || (this._initialized = !0, this._value = this._initializer.apply(null, i$4)); this._value instanceof Promise;) this._value = yield this._value;
					return e$1.unblock(void 0), this.queue.splice(this.queue.indexOf(e$1.promise), 1), this._value;
				}));
			}
			clear() {
				return n(this, void 0, void 0, (function* () {
					const i$4 = new t$1();
					return yield Promise.all(this.queue), this.queue.push(i$4.promise), this._initialized = !1, this._value = null, i$4.unblock(void 0), this.queue.splice(this.queue.indexOf(i$4.promise), 1), this._value;
				}));
			}
		}
		class u extends r {
			...
			get(...i$4) {
				return this._initialized || (this._initialized = !0, this._value = this._initializer.apply(null, i$4)), this._value;
			}
			clear() {
				return this._initialized = !1, this._value = null, this._value;
			}
		}
		const l = (() => n(void 0, void 0, void 0, (function* () {}))).constructor;
		function o(i$4) {
			return 1 == (i$4 instanceof l && l !== Function) ? new s(i$4) : new u(i$4);
		}
		module.exports = e;
	})();
})))());
var import_node_id3 = /* @__PURE__ */ __toESM(require_node_id3());
const getFolder = (customFolder) => customFolder ?? app.getPath("downloads");
const sendFeedback = (win$1, message) => {
	win$1.webContents.send("downloader-feedback", message);
};
const cropMaxWidth = (image) => {
	const imageSize = image.getSize();
	if (imageSize.width === 1280 && imageSize.height === 720) return image.crop({
		x: 280,
		y: 0,
		width: 720,
		height: 720
...
const setBadge = (n) => {
	if (import_is$1.default.linux() || import_is$1.default.macOS()) app.setBadgeCount(n);
};
var import_is = /* @__PURE__ */ __toESM(require_is());
var ffmpeg = (0, import_lib.lazy)(async () => (await import("./src-C9TKJmiF.js").then(__toDynamicImportESM())).createFFmpeg({
	log: false,
	logger() {},
	progress() {}
}));
var ffmpegMutex = new Mutex();
...
var playingUrl;
var isPremium = async () => {
	if (!await win.webContents.executeJavaScript("!!yt.config_.LOGGED_IN")) return false;
	const upgradeBtnIconPathData = await win.webContents.executeJavaScript("document.querySelector('iron-iconset-svg[name=\"yt-sys-icons\"] #youtube_music_monochrome')?.firstChild?.getAttribute(\"d\")?.substring(0, 15)");
	...
	return await win.webContents.executeJavaScript(`!document.querySelector('${upgradeButton}')`);
};
var sendError = (error, source) => {
...
const getCookieFromWindow = async (win$1) => {
	return (await win$1.webContents.session.cookies.get({ url: "https://music.youtube.com" })).map((it) => it.name + "=" + it.value).join(";");
};
var config;
const onMainLoad = async ({ window: _win, getConfig, ipc }) => {
	win = _win;
	...
	if (visitorData) {
		...
		try {
			...
			const window$1 = new (await (import("./lib-driZXbEQ.js"))).Window({
				width,
				height,
				console
			});
			...
			} else cleanUp(globalThis);
		} catch {
			cleanUp(globalThis);
		}
	}
	ipc.handle("download-song", (url) => downloadSong(url));
	ipc.on("peard:video-src-changed", (data) => {
		playingUrl = data.microformat.microformatDataRenderer.urlCanonical;
	});
	ipc.handle("download-playlist-request", async (url) => downloadPlaylist(url));
	downloadSongOnFinishSetup({
		ipc,
		getConfig
	});
};
...
async function downloadSong(url, playlistFolder = void 0, trackId = void 0, increasePlaylistProgress = () => {}) {
	let resolvedName;
	try {
		await downloadSongUnsafe(false, url, (name) => resolvedName = name, playlistFolder, trackId, increasePlaylistProgress);
	} catch (error) {
		sendError(error, resolvedName || url);
	}
}
async function downloadSongFromId(id, playlistFolder = void 0, trackId = void 0, increasePlaylistProgress = () => {}) {
	let resolvedName;
	try {
		await downloadSongUnsafe(true, id, (name) => resolvedName = name, playlistFolder, trackId, increasePlaylistProgress);
	} catch (error) {
		sendError(error, resolvedName || id);
	}
}
function downloadSongOnFinishSetup({ ipc }) {
	let currentUrl;
	let duration;
	let time = 0;
	...
	registerCallback((songInfo, event) => {
		...
		if (!songInfo.isPaused && songInfo.url !== currentUrl && config.downloadOnFinish?.enabled) {
			if (typeof currentUrl === "string" && duration && duration > 0) {
				if (config.downloadOnFinish.mode === "seconds" && duration - time <= config.downloadOnFinish.seconds) downloadSong(currentUrl, config.downloadOnFinish.folder ?? config.downloadFolder ?? defaultDownloadFolder);
				else if (config.downloadOnFinish.mode === "percent" && time >= duration * (config.downloadOnFinish.percent / 100)) downloadSong(currentUrl, config.downloadOnFinish.folder ?? config.downloadFolder ?? defaultDownloadFolder);
			}
...
async function downloadSongUnsafe(isId, idOrUrl, setName, playlistFolder = void 0, trackId = void 0, increasePlaylistProgress = () => {}) {
	const sendFeedback$1 = (message, progress) => {
		if (!playlistFolder) {
			sendFeedback(win, message);
			if (progress && !isNaN(progress)) win.setProgressBar(progress);
		}
	};
	sendFeedback$1(t("plugins.downloader.backend.feedback.downloading"), 2);
	let id;
	if (isId) id = idOrUrl;
	else {
		id = getVideoId(idOrUrl);
		if (typeof id !== "string") throw new Error(t("plugins.downloader.backend.feedback.video-id-not-found"));
	}
	let info = await yt.music.getInfo(id);
	if (!info) throw new Error(t("plugins.downloader.backend.feedback.video-id-not-found"));
	...
	if (playabilityStatus?.status === "LOGIN_REQUIRED") {
		...
		if (playabilityStatus?.status === "LOGIN_REQUIRED") throw new Error(`[${playabilityStatus.status}] ${playabilityStatus.reason}`);
		info = bypassedResult;
	}
	...
	let presetSetting;
	if (selectedPreset === "Custom") presetSetting = config.customPresetSetting ?? DefaultPresetList["Custom"];
	else if (selectedPreset === "Source") presetSetting = DefaultPresetList["Source"];
	else presetSetting = DefaultPresetList["mp3 (256kbps)"];
	...
	if (!presetSetting?.extension) targetFileExtension = VideoFormatList.find((it) => it.itag === format.itag)?.container ?? "mp3";
	else targetFileExtension = presetSetting?.extension ?? "mp3";
	let filename = filenamify(`${name}.${targetFileExtension}`, {
		replacement: "_",
		maxLength: 255
	});
	if (!import_is.default.macOS()) filename = filename.normalize("NFC");
	const filePath = join(dir, filename);
	if (config.skipExisting && existsSync(filePath)) {
		sendFeedback$1(null, -1);
		return;
	}
	const stream = await info.download(downloadOptions);
	console.info(t("plugins.downloader.backend.feedback.download-info", {
	...
	let fileBuffer = await iterableStreamToProcessedUint8Array(iterableStream, targetFileExtension, metadata, presetSetting?.ffmpegArgs ?? [], format.content_length ?? 0, sendFeedback$1, increasePlaylistProgress);
	if (fileBuffer && targetFileExtension === "mp3") fileBuffer = await writeID3(Buffer.from(fileBuffer), metadata, sendFeedback$1);
	if (fileBuffer) writeFileSync(filePath, fileBuffer);
	sendFeedback$1(null, -1);
	console.info(t("plugins.downloader.backend.feedback.done", { filePath }));
}
async function downloadChunks(stream, contentLength, sendFeedback$1, increasePlaylistProgress = () => {}) {
	const chunks = [];
	let downloaded = 0;
	for await (const chunk of stream) {
		...
		sendFeedback$1(t("plugins.downloader.backend.feedback.download-progress", { percent: progress }), ratio);
		increasePlaylistProgress(ratio * .15);
	}
	return chunks;
}
async function iterableStreamToProcessedUint8Array(stream, extension, metadata, presetFfmpegArgs, contentLength, sendFeedback$1, increasePlaylistProgress = () => {}) {
	sendFeedback$1(t("plugins.downloader.backend.feedback.loading"), 2);
	...
	return await ffmpegMutex.runExclusive(async () => {
		try {
			...
			sendFeedback$1(t("plugins.downloader.backend.feedback.preparing-file"));
			ffmpegInstance.FS("writeFile", safeVideoName, Buffer.concat(await downloadChunks(stream, contentLength, sendFeedback$1, increasePlaylistProgress)));
			sendFeedback$1(t("plugins.downloader.backend.feedback.converting"));
			ffmpegInstance.setProgress(({ ratio }) => {
				sendFeedback$1(t("plugins.downloader.backend.feedback.conversion-progress", { percent: Math.floor(ratio * 100) }), ratio);
				increasePlaylistProgress(.15 + ratio * .85);
			});
			const safeVideoNameWithExtension = `${safeVideoName}.${extension}`;
			try {
				await ffmpegInstance.run("-i", safeVideoName, ...presetFfmpegArgs, ...getFFmpegMetadataArgs(metadata), safeVideoNameWithExtension);
			} finally {
				ffmpegInstance.FS("unlink", safeVideoName);
			}
			sendFeedback$1(t("plugins.downloader.backend.feedback.saving"));
			try {
...
var getCoverBuffer = async (url) => {
	const nativeImage$1 = cropMaxWidth(await getImage(url));
	return nativeImage$1 && !nativeImage$1.isEmpty() ? nativeImage$1.toPNG() : null;
};
async function writeID3(buffer$1, metadata, sendFeedback$1) {
	try {
		sendFeedback$1(t("plugins.downloader.backend.feedback.writing-id3"));
		const tags = {};
		tags.title = metadata.title;
		tags.artist = metadata.artist;
		if (metadata.album) tags.album = metadata.album;
		const coverBuffer = await getCoverBuffer(metadata.imageSrc ?? "");
		if (coverBuffer) tags.image = {
			mime: "image/png",
			type: { id: import_node_id3.TagConstants.AttachedPicture.PictureType.FRONT_COVER },
			description: "thumbnail",
			imageBuffer: coverBuffer
		};
		if (metadata.trackId) tags.trackNumber = metadata.trackId;
		return import_node_id3.write(tags, buffer$1);
	} catch (error) {
...
async function downloadPlaylist(givenUrl) {
	...
	const playlistId = getPlaylistID(givenUrl) || getPlaylistID(new URL(playingUrl));
	if (!playlistId) {
		sendError(new Error(t("plugins.downloader.backend.feedback.playlist-id-not-found")));
		return;
	}
	const sendFeedback$1 = (message) => sendFeedback(win, message);
	console.log(t("plugins.downloader.backend.feedback.trying-to-get-playlist-id", { playlistId }));
	sendFeedback$1(t("plugins.downloader.backend.feedback.getting-playlist-info"));
	let playlist;
	const items = [];
	try {
		playlist = await yt.music.getPlaylist(playlistId);
		if (playlist?.items) {
			const filteredItems = playlist.items.filter((item) => item instanceof MusicResponsiveListItem);
			items.push(...filteredItems);
		}
	} catch (error) {
		sendError(Error(t("plugins.downloader.backend.feedback.playlist-is-mix-or-private", { error: String(error) })));
		return;
	}
	if (!playlist || !playlist.items || playlist.items.length === 0) {
		sendError(new Error(t("plugins.downloader.backend.feedback.playlist-is-empty")));
		return;
	}
	const normalPlaylistTitle = playlist.header && "title" in playlist.header ? playlist.header?.title?.text : void 0;
	const playlistTitle = normalPlaylistTitle ?? playlist.page.contents_memo?.get("MusicResponsiveListItemFlexColumn")?.at(2)?.as(MusicResponsiveListItemFlexColumn)?.title?.text ?? "NO_TITLE";
	...
	if (items.length === 1) {
		sendFeedback$1(t("plugins.downloader.backend.feedback.playlist-has-only-one-song"));
		await downloadSongFromId(items.at(0).id);
		return;
	}
	let safePlaylistTitle = filenamify(playlistTitle, { replacement: " " });
	if (!import_is.default.macOS()) safePlaylistTitle = safePlaylistTitle.normalize("NFC");
	const folder = getFolder(config.downloadFolder ?? "");
	const playlistFolder = join(folder, safePlaylistTitle);
	if (existsSync(playlistFolder)) {
		if (!config.skipExisting) {
			sendError(new Error(t("plugins.downloader.backend.feedback.folder-already-exists", { playlistFolder })));
			return;
		}
	} else mkdirSync(playlistFolder, { recursive: true });
	dialog.showMessageBox(win, {
		type: "info",
		...
		message: t("plugins.downloader.backend.dialog.start-download-playlist.message", { playlistTitle }),
		detail: t("plugins.downloader.backend.dialog.start-download-playlist.detail", { playlistSize: items.length })
	});
	if (import_is.default.dev()) console.log(t("plugins.downloader.backend.feedback.downloading-playlist", {
		playlistTitle,
	...
	const increaseProgress = (itemPercentage) => {
		const newProgress = (counter - 1) / (items.length ?? 1) + progressStep * itemPercentage;
		win.setProgressBar(newProgress);
	};
	try {
		for (const song of items) {
			sendFeedback$1(t("plugins.downloader.backend.feedback.downloading-counter", {
				current: counter,
				total: items.length
			}));
			const trackId = isAlbum ? counter : void 0;
			await downloadSongFromId(song.id, playlistFolder, trackId?.toString(), increaseProgress).catch((error) => sendError(new Error(t("plugins.downloader.backend.feedback.error-while-downloading", {
...
var getPlaylistID = (aURL) => {
	const result = aURL?.searchParams.get("list") || aURL?.searchParams.get("playlist");
	if (result?.startsWith("RDAMPL")) return result.slice(6);
	return result;
};
var getVideoId = (url) => {
	return new URL(url).searchParams.get("v");
};
var getMetadata = (info) => ({
	videoId: info.basic_info.id,
	title: cleanupName(info.basic_info.title),
	artist: cleanupName(info.basic_info.author),
	album: info.player_overlays?.browser_media_session?.as(BrowserMediaSession).album?.text,
	imageSrc: info.basic_info.thumbnail?.find((t$1) => !t$1.url.endsWith(".webp"))?.url,
	views: info.basic_info.view_count,
	songDuration: info.basic_info.duration,
	mediaType: MediaType.Audio
});
var getAndroidTvInfo = async (id) => {
	...
	return [
		{
			...
			submenu: [
				{
					label: t("plugins.downloader.menu.download-finish-settings.submenu.enabled"),
					type: "checkbox",
					checked: config$1.downloadOnFinish?.enabled ?? false,
					click(item) {
						setConfig({ downloadOnFinish: {
							...deepmerge(defaultConfig.downloadOnFinish, config$1.downloadOnFinish),
				...
				{ type: "separator" },
				{
					...
					click() {
						const result = dialog.showOpenDialogSync({
							...
							defaultPath: getFolder(config$1.downloadOnFinish?.folder ?? config$1.downloadFolder)
						});
						if (result) setConfig({ downloadOnFinish: {
							...deepmerge(defaultConfig.downloadOnFinish, config$1.downloadOnFinish),
							folder: result[0]
						} });
					}
				...
				{
					label: t("plugins.downloader.menu.download-finish-settings.submenu.mode"),
					type: "submenu",
					submenu: [{
						label: t("plugins.downloader.menu.download-finish-settings.submenu.seconds"),
						type: "radio",
						checked: config$1.downloadOnFinish?.mode === "seconds",
						click() {
							setConfig({ downloadOnFinish: {
								...deepmerge(defaultConfig.downloadOnFinish, config$1.downloadOnFinish),
								mode: "seconds"
						...
						label: t("plugins.downloader.menu.download-finish-settings.submenu.percent"),
						type: "radio",
						checked: config$1.downloadOnFinish?.mode === "percent",
						click() {
							setConfig({ downloadOnFinish: {
								...deepmerge(defaultConfig.downloadOnFinish, config$1.downloadOnFinish),
								mode: "percent"
				...
				{
					label: t("plugins.downloader.menu.download-finish-settings.submenu.advanced"),
					async click() {
						const res = await prompt({
							title: t("plugins.downloader.menu.download-finish-settings.prompt.title"),
							type: "multiInput",
							multiInputOptions: [{
								label: t("plugins.downloader.menu.download-finish-settings.prompt.last-seconds"),
								inputAttrs: {
									type: "number",
									required: true,
									min: "0",
								...
								value: config$1.downloadOnFinish?.seconds ?? defaultConfig.downloadOnFinish.seconds
							}, {
								label: t("plugins.downloader.menu.download-finish-settings.prompt.last-percent"),
								inputAttrs: {
									type: "number",
									required: true,
									min: "1",
								...
								value: config$1.downloadOnFinish?.percent ?? defaultConfig.downloadOnFinish.percent
							}],
		...
		},
		{
			...
			click() {
				const result = dialog.showOpenDialogSync({
					properties: ["openDirectory", "createDirectory"],
					defaultPath: getFolder(config$1.downloadFolder ?? "")
				});
				if (result) setConfig({ downloadFolder: result[0] });
			}
		},
		{
			...
			submenu: Object.keys(DefaultPresetList).map((preset) => ({
				label: preset,
				type: "radio",
				checked: config$1.selectedPreset === preset,
				click() {
...
const defaultConfig = {
	...
	customPresetSetting: DefaultPresetList["mp3 (256kbps)"],
	skipExisting: false,
	playlistMaxItems: void 0
};
var downloader_default = createPlugin({
...
export { downloader_default as default, defaultConfig, pluginStub };
