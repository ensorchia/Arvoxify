...
const main = {
	"console": {
		"did-finish-load": { "dev-tools": "Laadimine lõppes, arendaja tarvikud on avatud" },
		"i18n": { "loaded": "i18n on laaditud" },
		"second-instance": { "receive-command": "„{{command}}“ käsk on vastu võetud" },
		"theme": { "css-file-not-found": "CSS faili „{{cssFile}}“ pole olemas, seega eirame eelistust" },
		"unresponsive": { "details": "Tõrge ei vasta!\n{{error}}" },
		"when-ready": { "clearing-cache-after-20s": "Programmi vahemälu kustutamine" },
		"window": { "tried-to-render-offscreen": "Akent prooviti renderdada väljaspool ekraani, windowSize={{windowSize}}, displaySize={{displaySize}}, position={{position}}" }
	},
	"dialog": {
		"hide-menu-enabled": {
		...
		"options": {
			...
			"submenu": {
				...
				"language": {
					...
					"label": "Keel",
					"submenu": { "to-help-translate": "Soovid aidata tõlkimisel? Klõpsi siin" }
				},
...
const plugins = {
	...
	"ambient-mode": {
		...
		"menu": {
			"blur-amount": { "label": "Hägusus" },
			"opacity": { "submenu": { "percent": "{{opacity}}%" } },
			"quality": {
				"label": "Kvaliteet",
				"submenu": { "pixels": "{{quality}} pikslit" }
			},
			"size": {
				"label": "Suurus",
	...
	"lyrics-genius": {
		...
		"menu": { "romanized-lyrics": "Latiniseeritud laulusõnad" },
		"name": "Lyrics Genius",
		"renderer": { "fetched-lyrics": "Leidsime Geeniuse jaoks ühed laulusõnad" }
	},
	"navigation": { "name": "Liikumine" },
	"no-google-login": {
		"description": "Eemalda kasutajaliidesest Google'i sisselogimisnupud",
		"name": "Elu ilma Google'i sisselogimiseta"
	},
	"quality-changer": {
		"backend": { "dialog": { "quality-changer": {
	...
	"scrobbler": {
		...
		"dialog": { "lastfm": { "auth-failed": {
			"message": "Last.fm'i autentimine ei õnnestunud\nPeida hüpikaken järgmise taaskäivituseni.",
			"title": "Autentimine ei õnnestunud"
		} } },
		"menu": {
			"lastfm": { "api-settings": "Last.fm API seadistused" },
			"listenbrainz": { "token": "Sisesta ListenBrainz'i kasutaja tunnusluba" },
	...
	"synced-lyrics": { "menu": { "show-lyrics-even-if-inexact": { "tooltip": "Kui lugu ei leidu, siis lisamoodul üritab uut otsingut teistsuguse päringuga.\nTeise katse puhul tulemused ei pruugi olla väga täpsed." } } },
	"tuna-obs": { "description": "Lõimimine OBSi Tuna lisamooduliga" }
};
var et_default = {
	common,
	language,
...
export { common, et_default as default, language, main, plugins };
