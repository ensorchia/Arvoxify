import { n as __esmMin } from "./chunk-0rTXi_Jc.js";
import { a as init_from, c as init_esm_min, l as file_default, o as FormData } from "./from-BXT7MEZQ.js";
...
var s, S, f, F, lower, noop, MultipartParser;
__esmMin((() => {
	init_from();
	init_esm_min();
	s = 0;
				...
				switch (state) {
					case S.START_BOUNDARY:
						if (index === boundary.length - 2) {
							if (c === 45) flags |= F.LAST_BOUNDARY;
							else if (c !== 13) return;
							index++;
							break;
						} else if (index - 1 === boundary.length - 2) {
							if (flags & F.LAST_BOUNDARY && c === 45) {
								state = S.END;
								flags = 0;
							} else if (!(flags & F.LAST_BOUNDARY) && c === 10) {
								index = 0;
					...
					case S.PART_DATA:
						...
						if (index < boundary.length) if (boundary[index] === c) {
							...
							index++;
							if (c === 13) flags |= F.PART_BOUNDARY;
							else if (c === 45) flags |= F.LAST_BOUNDARY;
							else index = 0;
						...
						} else if (flags & F.LAST_BOUNDARY) if (c === 45) {
							callback("onPartEnd");
							state = S.END;
							flags = 0;
						} else index = 0;
