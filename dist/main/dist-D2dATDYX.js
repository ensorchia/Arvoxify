...
var getPattern = (label, next) => {
	...
	if (match) {
		...
		if (!patternCache[cacheKey$1]) if (match[2]) patternCache[cacheKey$1] = next && next[0] !== ":" && next[0] !== "*" ? [
			cacheKey$1,
			match[1],
...
var Node$1 = class {
	...
	insert(tokens, index, paramMap, context, pathErrorCheckOnly) {
		...
		if (pattern) {
			...
			if (!node) {
				if (Object.keys(this.#children).some((k) => k !== ".*" && k !== "(?:|/.*)")) throw PATH_ERROR;
				if (pathErrorCheckOnly) return;
			...
			if (!node) {
				if (Object.keys(this.#children).some((k) => k.length > 1 && k !== ".*" && k !== "(?:|/.*)")) throw PATH_ERROR;
				if (pathErrorCheckOnly) return;
...
var RegExpRouter = class {
	...
	#buildMatcher(method) {
		...
		[this.#middleware, this.#routes].forEach((r) => {
			...
			} else if (method !== "ALL") routes.push(...Object.keys(r[METHOD_NAME_ALL]).map((path) => [path, r[METHOD_NAME_ALL][path]]));
		});
...
export { raw as a, decodeURIComponent_ as c, escapeToBuffer as i, mergePath as l, cors as n, resolveCallbackSync as o, Hono as r, stringBufferToString as s, serve as t, tryDecode as u };
